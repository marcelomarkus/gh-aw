---
title: GitHub Tokens/Secrets
description: Comprehensive reference for all GitHub tokens (secrets) used in gh-aw, including authentication, token precedence, and security best practices
sidebar:
  order: 650
...
## Which tokens/secrets do I need?

You will need one of the following tokens/secrets:

- **Copilot** – Add [`COPILOT_GITHUB_TOKEN`](#copilot_github_token)
- **Claude by Anthropic** – Add [`ANTHROPIC_API_KEY`](/gh-aw/reference/engines/#using-claude-by-anthropic-claude-code)
- **Codex by OpenAI** – Add [`OPENAI_API_KEY`](/gh-aw/reference/engines/#using-openai-codex)

Depending on what your workflow needs to do, you may need additional tokens:
...
## How do I add tokens/secrets to my repository?

You can set up your tokens/secrets manually in the GitHub UI or use the CLI for a streamlined experience. Note that the CLI also provides commands to check existing secrets and validate token permissions.
